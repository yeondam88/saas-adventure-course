...
<input type="radio" id="{{ ucfirst($plan->name) }}-plan" value="{{ $plan->name }}" name="plan"
  @if(auth()->user()->plan->name == $plan->name) checked
@endif class="radio-plan hidden"/>
<label for="{{ ucfirst($plan->name) }}-plan"
  ...
  <div class="flex">
    ...
    <div>
      ...
      <span
        class="absolute right-0 bottom-0 bg-indigo-600 text-white font-bold rounded-br rounded-tl-lg text-sm px-2 py-1">
        @if($plan->name == 'basic')
        $7.00/mo.
